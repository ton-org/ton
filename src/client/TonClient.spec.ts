...
describeConditional("TonClient", () => {
    ...
    it("should get transactions", async () => {
        let transactions = await client.getTransactions(testAddress, {
            limit: 3,
        });
    ...
    it("should get extra currency info", async () => {
        ...
        let testAddr = Address.parse(
            "0:D36CFC9E0C57F43C1A719CB9F540ED87A694693AE1535B7654B645F52814AFD7",
        );
    ...
    it("should locate source/result tx", async () => {
        let source = Address.parse(
            "UQDDT0TOC4PMp894jtCo3-d1-8ltSjXMX2EuWww_pCNibsUH",
        );
        let createdLt = "37508996000002";

        let infoSource = await client.tryLocateSourceTx(
            source,
            testAddress,
            createdLt,
        );
        ...
        let infoResult = await client.tryLocateResultTx(
            source,
            testAddress,
            createdLt,
        );
