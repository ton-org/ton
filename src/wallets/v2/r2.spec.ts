...
import { WalletContractV2R2 } from "./r2";
...
describe("WalletContractV2R2", () => {
    it("should produce different transfer body when created with domain (signature differs)", () => {
        const key = randomTestKey("v2r2-domain");
        const walletDefault = WalletContractV2R2.create({
            workchain: 0,
            publicKey: key.publicKey,
        });
        const walletWithDomain = WalletContractV2R2.create({
            workchain: 0,
            publicKey: key.publicKey,
