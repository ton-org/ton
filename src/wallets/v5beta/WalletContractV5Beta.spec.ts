...
const getExtensionsArray = async (
    wallet: OpenedContract<WalletContractV5Beta>,
) => {
    try {
        ...
        if (
            ...
            typeof e === "object" &&
            "message" in e &&
            e.message === "Unsupported stack item type: list"
        ) {
            return [];
        }
        throw e;
...
describe.skip("WalletContractV5Beta", () => {
    ...
    it.skip("should has balance and correct address", async () => {
        ...
        expect(
            wallet.address.equals(
                Address.parse(
                    "EQAb3OyXDQyjOGf3HOXPXELTF8dhZP0onqMPz6fNQbleRqtY",
                ),
            ),
        ).toBeTruthy();
    ...
    it.skip("should add extension", async () => {
        ...
        const extensionContract = client.open(
            WalletContractV5Beta.create({
                walletId: { workchain: 0, networkGlobalId: -3 },
                publicKey: extensionKey.publicKey,
            }),
        );
        ...
        const extensionAlreadyAdded = extensions.some((address) =>
            address.equals(extensionContract.address),
        );
        ...
        if (!extensionAlreadyAdded) {
            ...
            const waitUntilExtensionAdded = async (
                attempt = 0,
            ): Promise<void> => {
                if (attempt >= 20) {
                ...
                const extensions = await getExtensionsArray(wallet);
                const extensionAdded = extensions.some((address) =>
                    address.equals(extensionContract.address),
                );
                if (extensionAdded) {
                    return;
    ...
    it.skip("should send internal transfer via relayer", async () => {
        ...
        const relayerContract = client.open(
            WalletContractV5Beta.create({
                walletId: { workchain: 0, networkGlobalId: -3 },
                publicKey: relaerKey.publicKey,
            }),
        );
    ...
    it.skip("should disable secret key auth, send extension-auth tx, and enable it again", async () => {
        ...
        const waitUntilAuthValue = async (
            target: "enabled" | "disabled",
            attempt = 0,
        ): Promise<void> => {
            if (attempt >= 20) {
                throw new Error(
                    "Auth permissions were not changed in 20 blocks",
                );
            }
            const isEnabledNow = await wallet.getIsSecretKeyAuthEnabled();
            if (
                (target === "enabled" && isEnabledNow) ||
                (target === "disabled" && !isEnabledNow)
            ) {
                return;
            }
