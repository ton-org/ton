...
export function storeOutListExtendedV5Beta(
    ...
    if (!action || !isOutActionExtended(action)) {
        ...
        return (builder: Builder) => {
            builder.storeUint(0, 1).storeRef(
                beginCell()
                    .store(storeOutList(actions as OutActionSendMsg[]))
                    .endCell(),
            );
        };
    }
    ...
    return (builder: Builder) => {
        builder
            ...
            .storeRef(
                beginCell().store(storeOutListExtendedV5Beta(rest)).endCell(),
            );
    };
}

export function loadOutListExtendedV5Beta(
    slice: Slice,
