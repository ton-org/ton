import { beginCell, BitReader, BitString, Builder, Slice } from "@ton/core";
...
const walletV5R1VersionsSerialisation: Record<
    WalletIdV5R1ClientContext["walletVersion"],
    number
> = {
    v5r1: 0,
};
 ...
 */
export function loadWalletIdV5R1(
    value: bigint | Buffer | Slice,
    ...
    if (isClientContext) {
        ...
        const walletVersion = Object.entries(
            walletV5R1VersionsSerialisation,
        ).find(([_, value]) => value === walletVersionRaw)?.[0] as
            | WalletIdV5R1ClientContext["walletVersion"]
            | undefined;
    ...
    return (builder: Builder) => {
        ...
        if (isWalletIdV5R1ClientContext(walletId.context)) {
            context = beginCell()
                ...
                .storeUint(
                    walletV5R1VersionsSerialisation[
                        walletId.context.walletVersion
                    ],
                    8,
                )
        ...
        return builder.storeInt(
            BigInt(walletId.networkGlobalId) ^ BigInt(context),
            32,
        );
    };
}
