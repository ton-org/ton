...
describe("Wallet V5R1 wallet id", () => {
    ...
    it("Should deserialise wallet id", () => {
        ...
        const actual = loadWalletIdV5R1(
            beginCell()
                .storeInt(
                    BigInt(context) ^ BigInt(expected.networkGlobalId),
                    32,
                )
                .endCell()
                .beginParse(),
            expected.networkGlobalId,
        );
            ...
            for (let testWc of [0, -1]) {
                for (let testSubwallet of [0, subwalletMax, ...randomBunch]) {
                    ...
                    const packed = beginCell()
                        .store(storeWalletIdV5R1(expected))
                        .endCell();

                    let unpacked = loadWalletIdV5R1(
                        packed.beginParse(),
                        networkId,
                    );
