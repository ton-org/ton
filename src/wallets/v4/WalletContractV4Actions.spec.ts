...
describe("Wallet V5R1 actions", () => {
    it("should serialize and deserialize OutActionSendMsg", async () => {
        ...
        const serialized = beginCell()
            .store(storeExtendedAction(msg))
            .endCell();
        ...
        expect(deserialized.messages[0].info.dest!.toString()).toEqual(
            msg.messages[0].info.dest!.toString(),
        );
    ...
    it("should serialize and deserialize OutActionAddAndDeployPlugin", () => {
        ...
        const serialized = beginCell()
            .store(storeExtendedAction(action))
            .endCell();
        const deserialized = loadExtendedAction(serialized.beginParse());
        ...
        expect(
            deserialized.stateInit.code?.equals(action.stateInit.code!),
        ).toBe(true);
        expect(
            deserialized.stateInit.data?.equals(action.stateInit.data!),
        ).toBe(true);
    });
