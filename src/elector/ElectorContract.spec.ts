...
const ecTwoPastElectionsEntries = client.openAt(
    BLOCK_WITH_TWO_PAST_ELECTIONS_ENTRIES,
    ElectorContract.create(),
);
const ecKnownBlock = client.openAt(KNOWN_BLOCK, ElectorContract.create());
const ecWithComaplints = client.openAt(
    BLOCK_WITH_COMPLAINTS,
    ElectorContract.create(),
);
...
describe("ElectorContract", () => {
    ...
    it("should return correct past elections records", async () => {
        const pastElections =
            await ecTwoPastElectionsEntries.getPastElections();
        ...
        const knownFrozenValue0 = pastElections[0].frozen.get(
            "12697811587540651918746850816771244166804229135431506663207437025351429985",
        );
        expect(
            knownFrozenValue0!["address"].equals(
                Address.parse(
                    "Ef-vmU4VjsKZhFfEvB-N_fXY8zcyH4ih6n9DcMtIAsy3YezN",
                ),
            ),
        ).toBe(true);
        ...
        const knownFrozenValue1 = pastElections[1].frozen.get(
            "216824161582481026645351194108767366817492989435791853445305829924424560264",
        );
        expect(
            knownFrozenValue1!["address"].equals(
                Address.parse(
                    "Ef_9j3g_jktlWpkCvQaEZ0qZ8qJH_fvyehUEAh0h5hZ1hCD6",
                ),
            ),
        ).toBe(true);
    ...
    it("should return correct election entities", async () => {
        ...
        expect(electionEntities!.entities[0].pubkey).toEqual(
            Buffer.from(
                "020a19785bb59d046bf1e62745263cf2cc91e5a47db997249b60c159b19443e7",
                "hex",
            ),
        );
        expect(electionEntities!.entities[0].stake).toEqual(380271797094836n);
        expect(
            electionEntities!.entities[0].address.equals(
                Address.parse(
                    "Ef8W1vCpA1tr9xr6QSXSxcVSdn1Sm7SYX_PCWQdClaWhales",
                ),
            ),
        ).toBe(true);
        expect(electionEntities!.entities[0].adnl).toEqual(
            Buffer.from(
                "1e7a93ab3274c5367c6ab8ea77790ef69df9af53657aa9da883238013aa7c03a",
                "hex",
            ),
        );
    });
    ...
    it("should return correct election entities", async () => {
        const complaints = await ecWithComaplints.getComplaints(
            ELECTIONS_ID_WITH_COMPLAUINTS,
        );
        expect(
            complaints[0].rewardAddress.equals(
                Address.parse(
                    "Ef9X6ObXojpUZza3NiS2TnRJ4KR7ler8cOjMRBt_swy4Qp2j",
                ),
            ),
        ).toBe(true);
        const actual = [];
        ...
        let reference = [
            {
                ...
                publicKey: Buffer.from(
                    "acacc7367fc6e8f3e82bb28d839361ee66f34f1e340eed8c82b169f2445ad3d5",
                    "hex",
                ),
            ...
            },
            {
                ...
                publicKey: Buffer.from(
                    "f5d09e351ca99e8850f393f294cc1ea7ae1ee73685fdec549903f8b7cadac48c",
                    "hex",
                ),
